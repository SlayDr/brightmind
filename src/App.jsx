...
function createSounds() {
  ...
  return {
    // Soft tap when pressing any button
    tap() {
      play(ctx => {
    ...
    // Big celebratory fanfare for quiz complete
    fanfare() {
      play(ctx => {
        const melody = [
          {f:523, t:0,    d:0.15},
...
const BANK = {
  ...
  english: [
    ...
    {q:"Which word is a pronoun?",options:["dog","run","she","blue"],answer:"she",hint:"She, he, it, they are pronouns — they replace names!"},
    {q:"Pick the conjunction (joining word):",options:["run","blue","and","fast"],answer:"and",hint:"'And' joins two things together: cats AND dogs!"},
    ...
    {q:"Which is the most exciting way to say 'walked'?",options:["walked slowly","went","stomped","moved"],answer:"stomped",hint:"Strong verbs make writing more exciting!"},
  ],
  reading: [
    ...
    {passage:"The library is a quiet place. People go there to read books and learn new things. You must whisper so you don't disturb others. Sam loves visiting every Saturday.",q:"When does Sam visit the library?",options:["Every day","Every Friday","Every Saturday","Every Sunday"],answer:"Every Saturday",hint:"Look at the last sentence!"},
    ...
    {passage:"Mia has three pets: a fluffy rabbit called Snowball, a goldfish called Bubbles, and a parrot named Polly. Polly can say 'hello' and 'goodnight'. Mia feeds them every morning.",q:"How many pets does Mia have?",options:["1","2","3","4"],answer:"3",hint:"Count the pets listed in the passage!"},
    ...
    {passage:"Jack loved space. He had posters of planets on his walls and could name all eight of them. His favourite was Saturn because of its beautiful rings. He wanted to be an astronaut one day.",q:"What did Jack want to be when he grew up?",options:["A scientist","A pilot","An astronaut","A teacher"],answer:"An astronaut",hint:"Look at the last sentence!"},
    ...
    {passage:"The old lighthouse stood on a rocky cliff. Every night its light flashed to warn ships of the dangerous rocks below. Without it, many ships might have crashed. The lighthouse keeper was very proud of his important job.",q:"Why did the lighthouse flash its light?",options:["To look pretty","To wake people up","To warn ships of rocks","To attract fish"],answer:"To warn ships of rocks",hint:"Read the second sentence!"},
    {passage:"The old lighthouse stood on a rocky cliff. Every night its light flashed to warn ships of the dangerous rocks below. Without it, many ships might have crashed. The lighthouse keeper was very proud of his important job.",q:"How did the lighthouse keeper feel about his job?",options:["Bored","Tired","Proud","Scared"],answer:"Proud",hint:"Look at the last sentence!"},
    {passage:"Penguins cannot fly but they are excellent swimmers. They use their wings as flippers to zoom through the water. A penguin can swim as fast as 25 kilometres per hour! They live in cold places like Antarctica.",q:"What do penguins use as flippers?",options:["Their feet","Their tails","Their wings","Their beaks"],answer:"Their wings",hint:"Read the second sentence!"},
    {passage:"Penguins cannot fly but they are excellent swimmers. They use their wings as flippers to zoom through the water. A penguin can swim as fast as 25 kilometres per hour! They live in cold places like Antarctica.",q:"Where do penguins live?",options:["Hot deserts","Cold places like Antarctica","Tropical rainforests","On mountains"],answer:"Cold places like Antarctica",hint:"Look at the last sentence!"},
    {passage:"On Saturday morning, the whole family went to the farmers' market. Dad bought fresh strawberries and Mum chose a bunch of sunflowers. The twins, Ella and Joe, each got a warm cinnamon roll. Everyone walked home smiling.",q:"What did Mum buy at the market?",options:["Strawberries","Sunflowers","Cinnamon rolls","Vegetables"],answer:"Sunflowers",hint:"Read the sentence about Mum!"},
    {passage:"On Saturday morning, the whole family went to the farmers' market. Dad bought fresh strawberries and Mum chose a bunch of sunflowers. The twins, Ella and Joe, each got a warm cinnamon roll. Everyone walked home smiling.",q:"How did everyone feel walking home?",options:["Tired","Upset","Smiling","Hungry"],answer:"Smiling",hint:"Look at the last sentence!"},
    {passage:"The giant tortoise is one of the longest-living animals on Earth. Some tortoises have lived for over 150 years! They move very slowly and eat plants. Their hard shell protects them from predators.",q:"What do giant tortoises eat?",options:["Fish","Insects","Plants","Meat"],answer:"Plants",hint:"Find the sentence about what they eat!"},
    {passage:"The giant tortoise is one of the longest-living animals on Earth. Some tortoises have lived for over 150 years! They move very slowly and eat plants. Their hard shell protects them from predators.",q:"What protects the tortoise from predators?",options:["Its speed","Its size","Its hard shell","Its colour"],answer:"Its hard shell",hint:"Look at the last sentence!"},
    {passage:"Maya wanted to make a card for her mum's birthday. She got out her art supplies: glitter, stickers, coloured paper and felt-tip pens. She spent an hour cutting and sticking. When she finished, her mum cried happy tears.",q:"Why did Maya make the card?",options:["For Christmas","For her teacher","For her mum's birthday","For a school project"],answer:"For her mum's birthday",hint:"Read the very first sentence!"},
    {passage:"Maya wanted to make a card for her mum's birthday. She got out her art supplies: glitter, stickers, coloured paper and felt-tip pens. She spent an hour cutting and sticking. When she finished, her mum cried happy tears.",q:"How long did Maya spend making the card?",options:["10 minutes","30 minutes","An hour","All day"],answer:"An hour",hint:"Find the sentence about time!"},
  ],
  verbal: [
...
@media(max-height:500px) and (orientation:landscape){.subject-grid{grid-template-columns:repeat(4,1fr)}.home-hero{padding:8px 0 6px}.logo{font-size:28px}}
@media(min-width:768px) and (orientation:landscape){.subject-grid{grid-template-columns:repeat(4,1fr)}}
...
.sound-btn{background:white;border:2px solid #e5e7eb;border-radius:99px;width:36px;height:36px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;flex-shrink:0;touch-action:manipulation}
.sound-btn:active{background:#f0f0f0}
...
/* ─── Home ─────────────────────────────────────────────────────────────── */
function Home({ progress, onSelect, sounds, muted, toggleMute }) {
  return (
    <div>
      <div className="home-hero">
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:4}}>
          <button className="sound-btn" onClick={toggleMute} title={muted?"Unmute":"Mute"}>
            {muted ? "🔇" : "🔊"}
          </button>
        </div>
...
function Quiz({ subjectId, onBack, onDone, sounds, muted, toggleMute }) {
  ...
  const next = () => {
    sounds.tap();
    if (idx+1>=TOTAL) { onDone(score+(sel===cur.answer?0:0)); setDone(true); }
  ...
  if (loading) return (
    <div>
      <div className="topbar">
        ...
        <button className="sound-btn" onClick={toggleMute}>{muted?"🔇":"🔊"}</button>
      </div>
  ...
  if (done) return <Results score={score} total={TOTAL} subject={s} onBack={() => { sounds.tap(); onBack(); }}
    onRetry={() => { sounds.tap(); setQs([]); setIdx(0); setSel(null); setHint(false); setScore(0); setDone(false); build(); }}
...
/* ─── Results ──────────────────────────────────────────────────────────── */
function Results({ score, total, subject:s, onBack, onRetry, sounds, muted, toggleMute }) {
...
export default function App() {
  ...
  // Create sounds once, wrap all calls to respect mute
  ...
  const sounds = {
    ...
    star:    () => { if (!muted) rawSounds.current.star(); },
  };
  ...
  return (
    <>
      ...
      <JungleBg/>
      <div className="app">
        {screen==="home"&&<Home progress={progress} onSelect={go} sounds={sounds} muted={muted} toggleMute={toggleMute}/>}
        {screen==="quiz"&&subject&&<Quiz subjectId={subject} onBack={home} onDone={done} sounds={sounds} muted={muted} toggleMute={toggleMute}/>}
      </div>
    </>
  );
}
